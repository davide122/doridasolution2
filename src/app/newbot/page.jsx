...
const WebBot = () => {
  ...
  const handleExportMessagePDF = (message) => {
    ...
    const lines = doc.splitTextToSize(content, margins.width);

    doc.text("Messaggio dell'Assistente", margins.left, y - 10);

    lines.forEach((line) => {
      if (y + 10 > doc.internal.pageSize.height - margins.bottom) {
        doc.addPage();
        y = margins.top;
      }
  ...
  const handleExportChatPDF = () => {
    ...
    messages.forEach((msg) => {
      const role = msg.role === "user" ? "User" : "Assistant";
      const content = convertHtmlToText(msg.content);
  ...
  if (!isLoggedIn) {
    return (
      <div className="container d-flex justify-content-center align-items-center flex-column vh-100 bg-light p-5 rounded-5">
        <form onSubmit={handleLogin}>
          ...
          <input
            ...
            className={styles.inputField}
          />
          <button type="submit" className="btn btn-primary w-100 rounded-5 my-3">
            Login
          </button>
  ...
  return (
    <div className={`container-fluid ${styles.chatContainer}`}>
      <div className={`row ${styles.fullHeight}`}>
        <div
          className={`col-md-2 col-0 d-none d-md-block ${styles.chatList} vh-100 p-3`}
        >
          <h3 className="text-center border-bottom p-2">Chat Disponibili</h3>
          <div className={`p-3 my-2 rounded ${styles.chatPlaceholder}`}>
        ...
        <div className={`col-md-10 col-12${styles.chatWindowContainer} vh-100`}>
          <div className={`card shadow-sm ${styles.chatWindow} vh-100`}>
            <div
              className={`card-header ${styles.cardHeader} d-flex justify-content-between align-items-center`}
            >
              <Image
              ...
              <DropdownButton
                ...
                className="dropdown"
              >
                <Dropdown.Item className="text-black" onClick={handleNewChat}>
                  Nuova Chat
                </Dropdown.Item>
                <Dropdown.Item className="text-black" onClick={handleExportChatPDF}>
                  Esporta Chat
                </Dropdown.Item>
                <Dropdown.Item className="text-black" onClick={handleShowIframeCode}>
            ...
            <div className={`card-body ${styles.chatMessages}`}>
              {renderMessages()}
              {isWait && <div className={styles.loadingDots}>{loadingDots}</div>}
              <div ref={messagesEndRef} />
            </div>
            <div
              className={`card-footer ${styles.chatInput} d-flex justify-content-center align-items-center flex-row`}
            >
              <input
                type="text"
                value={inputMessage}
